...
<div class="pand-console">
    <div class="pane-header">
        <h1 id="myHeader" class="trend-header">{news_create}</h1>
    </div>

    <form enctype="multipart/form-data" id="signup-form" action="/admin/news/create" method="post">
        <div class="container block blk-reg">
            <div class="inside">
                <span class="item">{gird_column_NewsTitle} : </span>
                <input name="news_title" id="news_title" type="text" value="<?= set_value('news_title'); ?>" /><br>
                &nbsp;<?= form_error('news_title'); ?><br>

                <span class="item">{gird_column_ShortDesc} : </span>
                <a id="news_short_desc_tinymce" targetURL="short_desc" href="#short_desc">{news_anchor_here_to_edit}</a><br>
                <?= form_error('news_short_desc'); ?><br>
                <textarea name="news_short_desc" id="news_short_desc" style="width:373px;height: 150px;display:none;"><?= set_value('news_short_desc'); ?></textarea>


                <span class="item">{news_cover_image} : </span>
                <div class="news_cover_image" style="background-image:url(<?= set_value('news_cover_image',''); ?>);background-position:<?= set_value('news_cover_image_position','0px 0px'); ?>;)">
                    {news_thumb} 
                </div><br>
                ...
                <input id='news_cover_image' name="news_cover_image" type="hidden" value="<?= set_value('news_cover_image'); ?>" />
                <input id='news_cover_image_thumb' name="news_cover_image_thumb" type="hidden" value="<?= set_value('news_cover_image_thumb'); ?>" />
                <input id='news_cover_image_position' name="news_cover_image_position" type="hidden" value="<?= set_value('news_cover_image_position'); ?>" />
                ...
                <textarea name="news_content" id="news_content" style="display:none;" ><?= set_value('news_content'); ?></textarea>
...
<script>
    ...
    $(function () {
        if( '<?= set_value('news_cover_image_thumb'); ?>' != ''){
            $('div.news_cover_image').html("&nbsp;").css({                //'background-image':'url(<?= set_value('news_cover_image'); ?>)',                //'background-position':'<?= set_value('news_cover_image_position'); ?>'            }).backgroundDraggable({                done:function(e){                    $('#news_cover_image_position').val($(e).css('backgroundPosition'));                }            });
        }
        $('#filemanager_news_cover_image' ).bind('click', function (e) {

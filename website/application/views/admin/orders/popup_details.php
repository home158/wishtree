...
<div class="pand-console">
    ...
    <div class="pane-header">
        <h1 id="myHeader" class="trend-header">{contextmenu_shipping}</h1>
    </div>
...
var O_PARENT = {
    menu : {
        ...
        CLIENT_MENU : {
            ...
            menu: [
                {
                    TEXT: '{contextmenu_save_but_not_ship}',
                    GROUP: 0,
                    BACKGROUND_POSITION: {
                        DEFAULT: [-0 ,-816],
                        GRAYOUT: [-0, -840]
                    ...
                    ONCLICK: function(e) {
                        ...
                        if ( $('input.product-item:checked').length == 0 ){
                            alert('{orders_check_one_to_save_but_not_ship}');
                            return false;
                        }else{
                            $('form[name=order_details]').attr("action",'/admin/orders/shipping/{order_GUID}/0');
                            $('form[name=order_details]').submit();
                        }
                    }
                },
                {
                    TEXT: '{contextmenu_save_and_ship}',
                    GROUP: 0,
                    BACKGROUND_POSITION: {
                        DEFAULT: [-0, -720],
                        GRAYOUT: [-0, -744]
                    ...
                    ONCLICK: function(e) {
                        if ( $('input.product-item:checked').length == 0 ){
                            ...
                            $('form[name=order_details]').attr("action",'/admin/orders/shipping/{order_GUID}/1');
                            $('form[name=order_details]').submit();
                        }
                ...
                /*
                {
                    ...
                    ONCLICK: function(e) {
                        if ( $('input.product-item:checked').length == 0 ){
                            ...
                            if(confirm('{orders_popup_delete_msg}')){
                                $('form[name=order_details]').attr("action",'/admin/orders/delete_details/{order_GUID}');
                                $('form[name=order_details]').submit();
                            }
                        }
                ...
                */
                {
                    TEXT: '{contextmenu_shipping_history_history}',
                    GROUP: 0,
                    DISABLED : false,
                    BACKGROUND_POSITION: {
                        DEFAULT: [-0, -672],
                    ...
                    ONCLICK: function(e) {
                        $( "#shipping_history_dialog" ).dialog( "open" );


                    }
                },
                {
                    TEXT: '{contextmenu_print_detail_all}',
                    GROUP: 0,
                    DISABLED : false,
                    BACKGROUND_POSITION: {
                        DEFAULT: [-0, -768],
                    ...
                    ONCLICK: function(e) {
                        $.popupWindow('/admin/orders/print_detail/{order_GUID}', { 
                            height: 600, 
                            width: 800,
                            createNew: true,
                            onUnload: function(){
        ...
        init: function(){
            this.$myGrid.trendGrid({
                columns: [
                    ...
                    {
                        id: "Remain",
                        name: '{gird_column_ShippingCountsRemain}',
                        width: 100,
                        sortAsc: false
                    },
                    {
